...
app.post('/api/appointments', (req, res) => {
  ...
  const smsBody = `${appointment.name}, 
                    this message is to confirm 
                    your appointment at 
                    ${time.format('h:mm a')} on 
                    ${date.format('dddd MMMM Do[,] YYYY')}.
                    Location: Krish Mall 6th Floor in Bugolobi.
                    Call +(256)078-274-6341 if you have any questions.`
  //send confirmation message to user
  twilioClient.messages.create({
    to: '+256' + appointment.phone,
    from: twilioNumber,
    body: smsBody
